        ...
        if f'def {func_name}()' in lines[i]:
            ...
            while j < len(lines) and 'def ' not in lines[j]:
                ...
                if 'rx.recharts.x_axis(type_="number", stroke="#94a3b8")' in lines[j]:
                    lines[j] = '                rx.recharts.x_axis(\n'
                    lines.insert(j+1, '                    type_="number",\n')
                    lines.insert(j+2, '                    stroke="#94a3b8",\n')
                ...
                # Y軸の置換
                    ...
                    lines[j] = '                rx.recharts.y_axis(\n'
                    lines.insert(j+1, '                    data_key="name",\n')
                    lines.insert(j+2, '                    type_="category",\n')
                    lines.insert(j+3, '                    width=120,\n')
                    lines.insert(j+4, '                    stroke="#94a3b8",\n')
...
print(f"\n日本語軸ラベルの一括追加が完了しました")
print(f"修正箇所: {modified_count}個のグラフ軸")
