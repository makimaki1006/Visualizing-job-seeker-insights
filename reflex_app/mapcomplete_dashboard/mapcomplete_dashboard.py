...
class DashboardState(rx.State):
    ...
    async def load_from_database(self):
        ...
        try:
            ...
            # Step 1: 都道府県リストのみ取得（軽量クエリ）
            prefectures_list = get_prefectures()

            if len(prefectures_list) > 0:
                # Step 2: 最初の都道府県を選択
                first_pref = prefectures_list[0]

                # Step 3: 市区町村リスト取得
                municipalities_list = get_municipalities(first_pref)

                # Step 4: 最初の市区町村を選択し、フィルタ済みデータのみ取得
                if len(municipalities_list) > 0:
                    first_muni = municipalities_list[0]
                    # フィルタ済みデータのみ取得（数十〜数百行）
                    df_data = self._normalize_df(get_filtered_data(first_pref, first_muni))
                    filtered_count = len(df_data) if df_data is not None else 0
                else:
                    first_muni = ""
                    # 市区町村がない場合は都道府県全体
                    df_data = self._normalize_df(get_filtered_data(first_pref))
                    filtered_count = len(df_data) if df_data is not None else 0
                ...
                if db_type == "csv":
                    ...
                    total_count = len(csv_df) if csv_df is not None else 0
                elif db_type == "turso":
                    count_df = query_df("SELECT COUNT(*) as cnt FROM job_seeker_data")
                    if not count_df.empty:
                ...
                # background=TrueではState更新をasync with self内で行う
                async with self:
                    self.prefectures = prefectures_list
                    self.selected_prefecture = first_pref
                ...
                print(f"[INFO] DB全体: {total_count:,}行, フィルタ済み: {filtered_count}行")
                print(f"[INFO] 都道府県数: {len(prefectures_list)}, 市区町村数: {len(municipalities_list)}")
            else:
    ...
    async def set_prefecture(self, value: str):
        ...
        if self.csv_uploaded and self.df_full is not None:
            ...
            if 'municipality' in self.df_full.columns:
                ...
                muni_list = filtered['municipality'].dropna().unique().tolist()
                municipalities_list = sorted([m for m in muni_list if m and str(m).lower() != 'nan'])
...
def persona_share_chart() -> rx.Component:
    """ペルソナ: 構成比横棒グラフ（円グラフから変更）

    Solution 2: 色覚バリアフリー対応のため円グラフから横棒グラフに変更
    GAS参照: Line 2721-2725
    """
    return rx.box(
        rx.recharts.bar_chart(
            ...
            rx.recharts.x_axis(
                type_="number",
                stroke="#94a3b8",
                label={"value": "構成比 (%)", "position": "insideBottom", "offset": -10, "style": {"fill": "#94a3b8", "fontSize": 12}}
            ),
            rx.recharts.y_axis(
                data_key="name",
                type_="category",
        ...
        rx.vstack(
            ...
            rx.cond(
                ...
                rx.vstack(
                    ...
                    # ペルソナ構成比（横棒グラフ）
                    rx.text("ペルソナ構成比（横棒グラフ）", font_size="0.9rem", color=MUTED_COLOR, font_weight="600", margin_top="2rem", margin_bottom="1rem"),
                    persona_share_chart(),
