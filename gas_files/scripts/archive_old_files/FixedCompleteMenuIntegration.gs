/**
 * 完全統合メニュー（Phase 1-7 + ネットワーク分析）
 *
 * 既存のGASメニューに全Phaseの機能を統合します。
 *
 * 作成日: 2025-10-27
 * バージョン: 2.1（関数名修正版）
 */

/**
 * カスタムメニュー作成（完全版）
 */
function onOpen() {
  const ui = SpreadsheetApp.getUi();

  // メインメニュー
  let menu = ui.createMenu('📊 データ処理');

  // ━━━ データインポートメニュー ━━━
  menu.addSubMenu(ui.createMenu('📥 データインポート')
    .addItem('📍 Phase 1: 基礎集計（4ファイル）', 'uploadPhase1')
    .addItem('📊 Phase 2: 統計分析（2ファイル）', 'uploadPhase2')
    .addItem('👥 Phase 3: ペルソナ分析（2ファイル）', 'uploadPhase3')
    .addItem('🌊 Phase 6: フロー分析（3ファイル）', 'uploadPhase6')
    .addItem('📈 Phase 7: 高度分析（7ファイル）', 'uploadPhase7')
    .addSeparator()
    .addItem('✅ 全Phaseアップロード状況', 'showAllPhasesUploadStatus')
  );

  menu.addSeparator();

  // ━━━ Phase 7高度分析メニュー ━━━
  const phase7Menu = ui.createMenu('📈 Phase 7高度分析')
    // 個別可視化機能
    .addSubMenu(ui.createMenu('📊 個別分析')
      .addItem('🗺️ 人材供給密度マップ', 'showSupplyDensityMap')
      .addItem('🎓 資格別人材分布', 'showQualificationDistribution')
      .addItem('👥 年齢層×性別クロス分析', 'showAgeGenderCrossAnalysis')
      .addItem('🚗 移動許容度スコアリング', 'showMobilityScoreAnalysis')
      .addItem('📊 ペルソナ詳細プロファイル', 'showDetailedPersonaProfile')
    )

    .addSeparator()

    // 統合ダッシュボード
    .addSubMenu(ui.createMenu('🎯 統合ダッシュボード')
      .addItem('📊 Phase 7統合ダッシュボード', 'showPhase7CompleteDashboard')
      .addItem('🌐 完全統合ダッシュボード（Phase 1+6+7+Network）', 'showCompleteIntegratedDashboard')
    )

    .addSeparator()

    // データ管理
    .addSubMenu(ui.createMenu('🔧 データ管理')
      .addItem('✅ データ検証', 'validatePhase7Data')
      .addItem('📊 データサマリー', 'showPhase7DataSummary')
      .addSeparator()
      .addItem('🧹 全データクリア', 'clearAllPhase7Data')
    )

    .addSeparator()

    // ヘルプ
    .addItem('❓ Phase 7クイックスタート', 'showPhase7QuickStart');

  menu.addSubMenu(phase7Menu);

  menu.addSeparator();

  // ━━━ 地図表示メニュー ━━━
  menu.addItem('🗺️ 地図表示（バブル）', 'showBubbleMap');
  menu.addItem('📍 地図表示（ヒートマップ）', 'showHeatMap');

  menu.addSeparator();

  // ━━━ ネットワーク分析メニュー ━━━
  menu.addItem('🌊 自治体間フロー分析', 'showMunicipalityFlowNetworkVisualization');

  menu.addSeparator();

  // ━━━ ペルソナ分析メニュー ━━━
  menu.addItem('🗺️ ペルソナマップ可視化', 'showPersonaMapVisualization');
  menu.addItem('🎯 ペルソナ難易度確認', 'showPersonaDifficultyChecker');

  menu.addToUi();
}

/**
 * Phase別アップロード関数
 */
function uploadPhase1() {
  showPhaseUploadDialog('phase1');
}

function uploadPhase2() {
  showPhaseUploadDialog('phase2');
}

function uploadPhase3() {
  showPhaseUploadDialog('phase3');
}

function uploadPhase6() {
  showPhaseUploadDialog('phase6');
}

function uploadPhase7() {
  showPhaseUploadDialog('phase7');
}

/**
 * クイックスタートガイド（更新版）
 */
function showPhase7QuickStart() {
  const ui = SpreadsheetApp.getUi();

  const message = `
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
Phase 1-7 完全版クイックスタート
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

【ステップ1】Pythonでデータ生成 🐍
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

1. コマンドプロンプトまたはターミナルで実行:
   cd "C:\\Users\\fuji1\\OneDrive\\Pythonスクリプト保管\\job_medley_project\\python_scripts"
   python run_complete.py

2. GUIダイアログでCSVファイルを選択

3. 出力確認:
   Phase 1: gas_output_phase1 (4ファイル)
   Phase 2: gas_output_phase2 (2ファイル)
   Phase 3: gas_output_phase3 (2ファイル)
   Phase 6: gas_output_phase6 (3ファイル)
   Phase 7: gas_output_phase7 (7ファイル)

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
【ステップ2】GASに一括アップロード 📤
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

メニュー: 📊 データ処理 > 📥 データインポート

Phase別にアップロード:
  📍 Phase 1: 基礎集計（4ファイル）
  📊 Phase 2: 統計分析（2ファイル）
  👥 Phase 3: ペルソナ分析（2ファイル）
  🌊 Phase 6: フロー分析（3ファイル）
  📈 Phase 7: 高度分析（7ファイル）

操作手順（各Phase共通）:
  1. ダイアログが開く
  2. 各エリアをクリックしてCSVファイルを選択
     （ドラッグ&ドロップも可能）
  3. 「アップロード開始」をクリック
  4. 完了！（自動でシート作成）

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
【ステップ3】可視化機能を使用 📊
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

🎯 完全統合ダッシュボード（推奨）:
  メニュー: 📈 Phase 7高度分析
          > 🎯 統合ダッシュボード
          > 🌐 完全統合ダッシュボード（Phase 1+6+7+Network）

  全機能をタブ切り替えで表示:
  - 📊 統合概要
  - 📍 Phase 1: 基礎集計
  - 🌊 Phase 6: フロー分析
  - 🔗 ネットワーク中心性
  - 🗺️ Phase 7: 供給密度
  - 🎓 Phase 7: 資格分布
  - 👥 Phase 7: 年齢×性別
  - 🚗 Phase 7: 移動許容度
  - 📋 Phase 7: ペルソナ

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
【アップロード状況確認】✅
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

メニュー: 📊 データ処理 > 📥 データインポート
        > ✅ 全Phaseアップロード状況

全18ファイルのアップロード状況を一覧表示

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
【トラブルシューティング】🔧
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

Q: メニューが表示されない
A: Googleスプレッドシートを完全に閉じて再度開く

Q: アップロードボタンが無効のまま
A: 全ファイルが選択されているか確認

Q: シートが作成されない
A: GASエディタのログを確認（表示 > ログ）

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
  `;

  ui.alert('Phase 1-7 完全版クイックスタート', message, ui.ButtonSet.OK);
}
