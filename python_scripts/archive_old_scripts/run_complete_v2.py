...
class SimpleJobSeekerAnalyzer:
    ...
    def _get_coords(self, prefecture, municipality=''):
        ...
        # キャッシュキー（既存キャッシュのパターンに合わせる）
        if municipality:
            # 既存キャッシュは"都道府県市区町村"形式（例: "京都府京都市山科区"）
        ...
        # キャッシュにない場合はNoneを返す（デフォルト座標は使わない）
        # デフォルト座標を使うと、すべての市区町村が同じ位置になってしまう
        return None, None
    ...
    def process_data(self):
        ...
        for idx, row in self.df.iterrows():
            ...
            processed_data.append({
                ...
                'career': row.get('career', '')  # Phase 8用: キャリア（学歴）情報
            })
    ...
    def export_phase1_data(self, output_dir='data/output_v2/phase1'):
        ...
        for location_key, count in location_counts.items():
            ...
            if pref:
                ...
                map_metrics_data.append({
                    'prefecture': pref,
                    'municipality': muni if muni else '',
        ...
        print(f"  [OK] Phase1_MapMetrics.csv: {len(map_metrics_df)}件")
        ...
        print(f"  [OK] Phase1_Applicants.csv: {len(applicants_df)}件")
        ...
        print(f"  [OK] Phase1_DesiredWork.csv: {len(desired_work_df)}件")

        # 4. AggDesired.csv - 希望勤務地集計（座標なし）- MapComplete用には不要なため削除
    ...
    def export_phase2_data(self, output_dir='data/output_v2/phase2'):
        ...
        chi_square_df.to_csv(output_path / 'Phase2_ChiSquareTests.csv', index=False, encoding='utf-8-sig')
        print(f"  [OK] Phase2_ChiSquareTests.csv: {len(chi_square_df)}件")
        ...
        anova_df.to_csv(output_path / 'Phase2_ANOVATests.csv', index=False, encoding='utf-8-sig')
        print(f"  [OK] Phase2_ANOVATests.csv: {len(anova_df)}件")

        print(f"  [DIR] 出力先: {output_path}")
        return True

    def export_phase3_data(self, output_dir='data/output_v2/phase3'):
        ...
        persona_summary_df.to_csv(output_path / 'Phase3_PersonaSummary.csv', index=False, encoding='utf-8-sig')
        print(f"  [OK] Phase3_PersonaSummary.csv: {len(persona_summary_df)}件")

        persona_details_df = pd.DataFrame(persona_details_data)
        persona_details_df.to_csv(output_path / 'Phase3_PersonaDetails.csv', index=False, encoding='utf-8-sig')
        print(f"  [OK] Phase3_PersonaDetails.csv: {len(persona_details_df)}件")

        print(f"  [DIR] 出力先: {output_path}")
        return True

    def export_phase6_data(self, output_dir='data/output_v2/phase6'):
        ...
        edges_df.to_csv(output_path / 'Phase6_MunicipalityFlowEdges.csv', index=False, encoding='utf-8-sig')
        print(f"  [OK] Phase6_MunicipalityFlowEdges.csv: {len(edges_df)}件")
        ...
        nodes_df.to_csv(output_path / 'Phase6_MunicipalityFlowNodes.csv', index=False, encoding='utf-8-sig')
        print(f"  [OK] Phase6_MunicipalityFlowNodes.csv: {len(nodes_df)}件")
        ...
        proximity_df.to_csv(output_path / 'Phase6_ProximityAnalysis.csv', index=False, encoding='utf-8-sig')
        print(f"  [OK] Phase6_ProximityAnalysis.csv: {len(proximity_df)}件")

        print(f"  [DIR] 出力先: {output_path}")
        return True

    def export_phase7_data(self, output_dir='data/output_v2/phase7'):
        ...
        supply_density_df.to_csv(output_path / 'Phase7_SupplyDensityMap.csv', index=False, encoding='utf-8-sig')
        print(f"  [OK] Phase7_SupplyDensityMap.csv: {len(supply_density_df)}件")
        ...
        # 3. AgeGenderCross.csv - 年齢×性別クロス分析（市区町村レベル）
        age_gender_cross_data = []

        # 市区町村×年齢層×性別でグループ化
        for (residence_pref, residence_muni), location_group in self.processed_data.groupby(['residence_pref', 'residence_muni']):
            location = f"{residence_pref}{residence_muni}"
            lat, lng = self._get_coords(residence_pref, residence_muni)
                ...
                for gender in ['男性', '女性']:
                    ...
                    if len(segment_data) > 0:
                        age_gender_cross_data.append({
                            ...
                            'count': len(segment_data),
                            'avg_age': segment_data['age'].mean(),
                            'avg_desired_areas': segment_data['desired_areas'].apply(len).mean(),
                            'avg_qualifications': segment_data['qualification_count'].mean(),
                            'national_license_rate': (segment_data['has_national_license']).mean(),
                            'employment_rate': (segment_data['employment_status'].notna()).mean() if 'employment_status' in segment_data.columns else None,
                            'latitude': lat,
                            'longitude': lng
                        })

        age_gender_cross_df = pd.DataFrame(age_gender_cross_data)
        print(f"  [DEBUG] Generated {len(age_gender_cross_data)} AgeGenderCross records before DataFrame conversion")
        print(f"  [DEBUG] DataFrame shape: {age_gender_cross_df.shape}")
        print(f"  [DEBUG] DataFrame columns: {list(age_gender_cross_df.columns)}")
        age_gender_cross_df.to_csv(output_path / 'Phase7_AgeGenderCrossAnalysis.csv', index=False, encoding='utf-8-sig')
        print(f"  [OK] Phase7_AgeGenderCrossAnalysis.csv: {len(age_gender_cross_df)}件（市区町村レベル）")
        ...
        mobility_score_df.to_csv(output_path / 'Phase7_MobilityScore.csv', index=False, encoding='utf-8-sig')
        print(f"  [OK] Phase7_MobilityScore.csv: {len(mobility_score_df)}件")

        # 5. PersonaProfile.csv - ペルソナ詳細プロファイル（市区町村レベル）
        persona_profile_data = []

        # 市区町村×年齢層×性別×就業状況でグループ化
        for (residence_pref, residence_muni), location_group in self.processed_data.groupby(['residence_pref', 'residence_muni']):
            location = f"{residence_pref}{residence_muni}"
            lat, lng = self._get_coords(residence_pref, residence_muni)
            ...
            for age_bucket in ['20代', '30代', '40代', '50代', '60代', '70歳以上']:
                for gender in ['男性', '女性']:
                    for employment_status in location_group['employment_status'].dropna().unique():
                        segment_data = location_group[
                            (location_group['age_bucket'] == age_bucket) &
                            (location_group['gender'] == gender) &
                            (location_group['employment_status'] == employment_status)
                        ]
                        ...
                        if len(segment_data) > 0:
                            persona_profile_data.append({
                                ...
                                'prefecture': residence_pref,
                                'municipality': residence_muni,
                                'persona_name': f'{age_bucket}・{gender}・{employment_status}',
                                'age_group': age_bucket,
                                'gender': gender,
                                ...
                                'avg_qualifications': segment_data['qualification_count'].mean(),
                                'national_license_rate': (segment_data['has_national_license']).mean(),
                                'avg_mobility_score': segment_data.apply(
        ...
        persona_profile_df.to_csv(output_path / 'Phase7_DetailedPersonaProfile.csv', index=False, encoding='utf-8-sig')
        print(f"  [OK] Phase7_DetailedPersonaProfile.csv: {len(persona_profile_df)}件（市区町村レベル）")
    ...
    def export_phase8_data(self, output_dir='data/output_v2/phase8'):
        """Phase 8: キャリア・学歴分析（市区町村レベル）"""
        print("\n[PHASE8] Phase 8: キャリア・学歴分析（市区町村レベル）")
        output_path = Path(output_dir)
        output_path.mkdir(parents=True, exist_ok=True)

        # CareerAgeCross_Matrix.csv - キャリア×年齢クロス（市区町村レベル）
        career_age_cross_data = []
        ...
        for (residence_pref, residence_muni), location_group in self.processed_data.groupby(['residence_pref', 'residence_muni']):
            ...
            # キャリア（卒業年または職歴）の取得
            if 'career' in location_group.columns:
                career_col = 'career'
            elif 'graduation_year' in location_group.columns:
                career_col = 'graduation_year'
            else:
                continue

            for career_value in location_group[career_col].dropna().unique():
                # 空文字列はスキップ
                if not career_value or str(career_value).strip() == '':
                    continue

                for age_bucket in ['20代', '30代', '40代', '50代', '60代', '70歳以上']:
                    segment_data = location_group[
                        (location_group[career_col] == career_value) &
                        (location_group['age_bucket'] == age_bucket)
                    ]

                    if len(segment_data) > 0:
                        career_age_cross_data.append({
                            'location': location,
                            'prefecture': residence_pref,
                            'municipality': residence_muni,
                            'career': career_value,
        ...
        career_age_cross_df = pd.DataFrame(career_age_cross_data)
        career_age_cross_df.to_csv(output_path / 'Phase8_CareerAgeCross_Matrix.csv', index=False, encoding='utf-8-sig')
        print(f"  [OK] CareerAgeCross_Matrix.csv: {len(career_age_cross_df)}件（市区町村レベル）")

        print(f"  [DIR] 出力先: {output_path}")
        return True

    def export_phase10_data(self, output_dir='data/output_v2/phase10'):
        """Phase 10: 転職意欲・緊急度分析（市区町村レベル）"""
        print("\n[PHASE10] Phase 10: 転職意欲・緊急度分析（市区町村レベル）")
        output_path = Path(output_dir)
        output_path.mkdir(parents=True, exist_ok=True)

        # urgency_scoreカラムが存在するか確認
        if 'urgency_score' not in self.processed_data.columns:
            print("  [WARN] urgency_scoreカラムが存在しません。Phase 10データ生成をスキップします。")
            return False
        ...
        self.processed_data['urgency_rank'] = self.processed_data['urgency_score'].apply(get_urgency_rank)

        # 1. UrgencyAgeCross.csv - 緊急度×年齢クロス（市区町村レベル）
        urgency_age_cross_data = []

        for (residence_pref, residence_muni), location_group in self.processed_data.groupby(['residence_pref', 'residence_muni']):
            location = f"{residence_pref}{residence_muni}"
            lat, lng = self._get_coords(residence_pref, residence_muni)

            for urgency_rank in location_group['urgency_rank'].dropna().unique():
                for age_bucket in ['20代', '30代', '40代', '50代', '60代', '70歳以上']:
                    segment_data = location_group[
                        (location_group['urgency_rank'] == urgency_rank) &
                        (location_group['age_bucket'] == age_bucket)
                    ]

                    if len(segment_data) > 0:
                        urgency_age_cross_data.append({
                            'location': location,
                            'prefecture': residence_pref,
                            'municipality': residence_muni,
                            ...
                            'count': len(segment_data),
                            'avg_age': segment_data['age'].mean(),
                            'avg_urgency_score': segment_data['urgency_score'].mean(),
                            'latitude': lat,
                            'longitude': lng
                        })

        urgency_age_cross_df = pd.DataFrame(urgency_age_cross_data)
        urgency_age_cross_df.to_csv(output_path / 'Phase10_UrgencyAgeCross.csv', index=False, encoding='utf-8-sig')
        print(f"  [OK] UrgencyAgeCross.csv: {len(urgency_age_cross_df)}件（市区町村レベル）")

        # 2. UrgencyEmploymentCross.csv - 緊急度×就業状況クロス（市区町村レベル）
        urgency_employment_cross_data = []
        ...
        for (residence_pref, residence_muni), location_group in self.processed_data.groupby(['residence_pref', 'residence_muni']):
            ...
            for urgency_rank in location_group['urgency_rank'].dropna().unique():
                for employment_status in location_group['employment_status'].dropna().unique():
                    ...
                    if len(segment_data) > 0:
                        urgency_employment_cross_data.append({
                            'location': location,
                            'prefecture': residence_pref,
                            'municipality': residence_muni,
                            'urgency_rank': urgency_rank,
        ...
        urgency_employment_cross_df = pd.DataFrame(urgency_employment_cross_data)
        urgency_employment_cross_df.to_csv(output_path / 'Phase10_UrgencyEmploymentCross.csv', index=False, encoding='utf-8-sig')
        print(f"  [OK] UrgencyEmploymentCross.csv: {len(urgency_employment_cross_df)}件（市区町村レベル）")
...
def main():
    ...
    # ファイル選択（コマンドライン引数またはGUI）
    if len(sys.argv) > 1:
        filepath = sys.argv[1]
        print(f"[INFO] コマンドライン引数からファイル読み込み: {filepath}")
    else:
        root = tk.Tk()
        root.withdraw()
    ...
    analyzer.export_phase8_data()
    analyzer.export_phase10_data()

    print(f"\n{'='*60}")
    print(f"  [OK] 全Phase完了（Phase 1-10）")
    print(f"{'='*60}\n")

    # MapComplete統合CSV生成（row_type形式、市区町村レベル）
    print(f"\n{'='*60}")
    print(f"  MapComplete統合CSV生成（row_type形式、市区町村レベル）")
    print(f"{'='*60}")

    try:
        from generate_mapcomplete_complete_sheets import MapCompleteCompleteSheetGenerator
        generator_mapcomplete = MapCompleteCompleteSheetGenerator()
        generator_mapcomplete.load_all_phases()
        generator_mapcomplete.generate_complete_sheets()
        print(f"  [OK] MapComplete統合CSV生成完了（row_type形式）")
    except Exception as e:
        print(f"  [ERROR] MapComplete統合CSV生成エラー: {e}")

    # PersonaLevel統合CSV生成（ペルソナセグメントレベル）
    print(f"\n{'='*60}")
    print(f"  PersonaLevel統合CSV生成（ペルソナセグメントレベル）")
    print(f"{'='*60}")

    try:
        from generate_persona_level_sheets import PersonaLevelSheetGenerator
        generator_persona = PersonaLevelSheetGenerator()
        generator_persona.load_all_phases()
        generator_persona.generate_prefecture_sheets()
        print(f"  [OK] PersonaLevel統合CSV生成完了")
    except Exception as e:
        print(f"  [ERROR] PersonaLevel統合CSV生成エラー: {e}")

    print(f"\n{'='*60}")
    print(f"  [✅ 完了] すべての処理が完了しました！")
    print(f"")
    print(f"  生成されたファイル:")
    print(f"  - Phase 1-7 CSV: data/output_v2/phase1/, phase2/, ...")
    print(f"  - MapComplete統合CSV: data/output_v2/mapcomplete_sheets/")
    print(f"  - PersonaLevel統合CSV: data/output_v2/persona_level_sheets/")
    print(f"{'='*60}\n")
