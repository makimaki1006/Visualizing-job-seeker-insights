...
function fetchApplicantsForMunicipality(prefecture, municipality) {
  ...
  // 1. DesiredWorkシートを読み込み
  const desiredWork = readFirstAvailableSheet(REGION_DASHBOARD_SHEETS.phase1.desiredWork);
  ...
  // 3. Applicantsシートを読み込み
  const allApplicants = readFirstAvailableSheet(REGION_DASHBOARD_SHEETS.phase1.applicants);
...
function fetchPhase6Flow(prefecture, municipality) {
  ...
  // AggregatedFlowEdgesシートを読み込み
  const allFlows = readFirstAvailableSheet(REGION_DASHBOARD_SHEETS.phase6.aggregatedFlowEdges);
  const flowNodes = readFirstAvailableSheet(REGION_DASHBOARD_SHEETS.phase6.flowNodes);
  const proximity = readFirstAvailableSheet(REGION_DASHBOARD_SHEETS.phase6.proximity);
