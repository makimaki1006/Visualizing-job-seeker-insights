...
def refactor_file(filepath, phase_name):
    ...
    print(f'  リファクタリング後: {after_lines}行 ({after_lines - before_stats["lines"]:+d}行)')
...
def main():
    ...
    files = [
        ('Phase1-6UnifiedVisualizations.gs', 'Phase 1-6'),
        ('DataServiceProvider.gs', 'Data Service'),
        ('QualityAndRegionDashboards.gs', 'Quality & Region'),
        ('DataImportAndValidation.gs', 'Import & Validation'),
    ]
    ...
    print(f'\n総行数: {total_before}行 → {total_after}行 ({total_after - total_before:+d}行)')
    ...
    for result in results:
        ...
        diff = after - before
        print(f'\n{result["file"]}:')
        print(f'  {before}行 → {after}行 ({diff:+d}行)')
