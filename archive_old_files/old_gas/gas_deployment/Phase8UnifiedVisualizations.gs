...
function loadCareerDistData() {
  const ss = SpreadsheetApp.getActiveSpreadsheet();
  const sheet = findSheetByNames(ss, generateSheetNameCandidates('CareerDistribution', 8));

  if (!sheet) {
    throw new Error('CareerDistributionシートが見つかりません');
  }
...
function loadCareerAgeCrossData() {
  const ss = SpreadsheetApp.getActiveSpreadsheet();
  const sheet = findSheetByNames(ss, generateSheetNameCandidates('CareerAgeCross', 8));

  if (!sheet) {
    throw new Error('CareerAgeCrossシートが見つかりません');
  }
...
function loadCareerAgeMatrixData() {
  const ss = SpreadsheetApp.getActiveSpreadsheet();
  const sheet = findSheetByNames(ss, generateSheetNameCandidates('CareerAgeMatrix', 8));

  if (!sheet) {
    throw new Error('CareerAgeMatrixシートが見つかりません');
  }
...
function loadGraduationYearData() {
  const ss = SpreadsheetApp.getActiveSpreadsheet();
  const sheet = findSheetByNames(ss, generateSheetNameCandidates('GradYearDist', 8));

  if (!sheet) {
    throw new Error('GradYearDistシートが見つかりません');
  }
