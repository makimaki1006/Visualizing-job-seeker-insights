...
function onOpen() {
  ...
  ui.createMenu('📊 データ処理')
    ...
    .addSeparator()
    // MapComplete統合ダッシュボード（row_type形式・全Phase統合）
    .addItem('🎯 MapComplete統合ダッシュボード（全Phase統合）', 'showMapCompleteIntegratedDashboard')
    .addSeparator()
...
/**
 * MapComplete統合ダッシュボード表示（row_type形式・全Phase統合）
 */
function showMapCompleteIntegratedDashboard() {
  var html = HtmlService.createHtmlOutputFromFile('map_complete_integrated')
    .setWidth(1600)
    .setHeight(1000);
  SpreadsheetApp.getUi().showModalDialog(html, '🎯 MapComplete統合ダッシュボード（全Phase統合）');
}

/**
 * Phase 12-14統合ダッシュボード表示（旧形式・Phase別シート）
 */
