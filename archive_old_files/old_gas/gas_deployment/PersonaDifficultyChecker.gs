...
function getMunicipalityList() {
  ...
  var sheet = findSheetByNames(ss, ['Phase3_PersonaSummaryByMunicipality', 'PersonaSummaryByMunicipality', 'Phase3_PersonaByMunicipality']);

  if (!sheet || sheet.getLastRow() <= 1) {
    return { success: false, message: 'PersonaSummaryByMunicipalityデータが見つかりません' };
  }
...
function getPersonaDataForDifficulty() {
  var ss = SpreadsheetApp.getActiveSpreadsheet();
  var summarySheet = findSheetByNames(ss, ['Phase3_PersonaSummary', 'PersonaSummary']);
  var detailsSheet = findSheetByNames(ss, ['Phase3_PersonaDetails', 'PersonaDetails']);
  var applicantsSheet = findSheetByNames(ss, ['Phase1_Applicants', 'Applicants']);
