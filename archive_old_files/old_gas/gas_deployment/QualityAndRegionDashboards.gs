...
const REGION_DASHBOARD_SHEETS = {
  phase1: {
    ...
    desiredWork: ['Phase1_DesiredWork', 'DesiredWork'],
    applicants: ['Phase1_Applicants', 'Applicants'],
  ...
  phase3: {
    ...
    cross: ['Phase3_PersonaSummaryByMunicipality', 'PersonaSummaryByMunicipality']
  },
  phase6: {
    flowEdges: ['Phase6_FlowEdges', 'MunicipalityFlowEdges', 'Phase6_MunicipalityFlowEdges'],
    flowNodes: ['Phase6_FlowNodes', 'MunicipalityFlowNodes', 'Phase6_MunicipalityFlowNodes'],
    aggregatedFlowEdges: ['Phase6_AggregatedFlowEdges', 'AggregatedFlowEdges'],
    proximity: ['Phase6_Proximity', 'ProximityAnalysis', 'Phase6_ProximityAnalysis'],
    quality: ['Phase6_QualityReport_Inferential', 'P6_QualityReport_Inferential', 'QualityReport_Inferential']
  },
  phase7: {
    supply: ['Phase7_SupplyDensity', 'SupplyDensityMap'],
    qualification: ['Phase7_QualificationDist', 'QualificationDistribution'],
    ageGender: ['Phase7_AgeGenderCross', 'AgeGenderCrossAnalysis'],
    mobility: ['Phase7_MobilityScore', 'MobilityScore'],
    personaProfile: ['Phase7_PersonaProfile', 'DetailedPersonaProfile'],
    personaMobility: ['Phase7_PersonaMobilityCross', 'PersonaMobilityCross'],
    personaMap: ['Phase7_PersonaMapData', 'PersonaMapData'],
  ...
  phase8: {
    ...
    educationCross: ['Phase8_EduAgeCross', 'EducationAgeCross'],
    educationMatrix: ['Phase8_EduAgeMatrix', 'EducationAgeCross_Matrix'],
    graduation: ['Phase8_GradYearDist', 'GraduationYearDistribution'],
    career: ['Phase8_CareerDistribution', 'CareerDistribution'],
    careerCross: ['Phase8_CareerAgeCross', 'CareerAgeCross'],
    careerMatrix: ['Phase8_CareerAgeMatrix', 'CareerAgeCross_Matrix'],
    quality: ['Phase8_QualityReport', 'Phase8_QualityReport_Inferential', 'P8_QualityReport', 'P8_QualityReport_Inferential']
  },
  phase10: {
    urgency: ['Phase10_UrgencyDist', 'UrgencyDistribution'],
    ageCross: ['Phase10_UrgencyAge', 'UrgencyAgeCross'],
    ageMatrix: ['Phase10_UrgencyAge_Matrix', 'UrgencyAgeCross_Matrix'],
    employmentCross: ['Phase10_UrgencyEmployment', 'UrgencyEmploymentCross'],
    employmentMatrix: ['Phase10_UrgencyEmployment_Matrix', 'UrgencyEmploymentCross_Matrix'],
    desiredWorkCross: ['Phase10_UrgencyDesiredWork', 'UrgencyDesiredWorkCross'],
    byMunicipality: ['Phase10_UrgencyByMunicipality', 'UrgencyByMunicipality'],
    quality: ['Phase10_QualityReport', 'Phase10_QualityReport_Inferential', 'P10_QualityReport', 'P10_QualityReport_Inferential']
  }
};
...
function fetchPhase7Supply(prefecture, municipality) {
  ...
  const personaSummaryByMunicipality = readFirstAvailableSheet(REGION_DASHBOARD_SHEETS.phase3.cross);
  const personaMobility = filterByRegion(
    readFirstAvailableSheet(REGION_DASHBOARD_SHEETS.phase7.personaMobility),
    ctx,
    REGION_FILTER_MAPPINGS.municipalityOnly,
    warnings,
    'PersonaMobilityCross'
  );
  const personaMap = filterByRegion(
    readFirstAvailableSheet(REGION_DASHBOARD_SHEETS.phase7.personaMap),
    ctx,
    REGION_FILTER_MAPPINGS.municipalityOnly,
    warnings,
    'PersonaMapData'
  ...
  return {
    ...
    tables: {
      ...
      personaMobility: personaMobility,
      personaMapData: personaMap,
      personaSummaryByMunicipality: filterByRegionIfPossible(personaSummaryByMunicipality, ctx, REGION_FILTER_MAPPINGS.municipalityOnly),
...
function fetchPhase8Education(prefecture, municipality) {
  ...
  const educationMatrix = readFirstAvailableSheet(REGION_DASHBOARD_SHEETS.phase8.educationMatrix);
  const graduation = readFirstAvailableSheet(REGION_DASHBOARD_SHEETS.phase8.graduation);
  const career = readFirstAvailableSheet(REGION_DASHBOARD_SHEETS.phase8.career);
  const careerCross = readFirstAvailableSheet(REGION_DASHBOARD_SHEETS.phase8.careerCross);
  const careerMatrix = readFirstAvailableSheet(REGION_DASHBOARD_SHEETS.phase8.careerMatrix);
  ...
  if (career.length) {
    warnings.push('CareerDistribution は地域列を持たないため、地域別フィルタリングは未対応です。');
  }
...
function fetchPhase10Urgency(prefecture, municipality) {
  ...
  const urgency = filterByRegion(
    ...
    ctx,
    REGION_FILTER_MAPPINGS.phase10Urgency,
    warnings,
    'UrgencyDistribution'
  );
  ...
  const ageMatrix = filterByRegion(
    readFirstAvailableSheet(REGION_DASHBOARD_SHEETS.phase10.ageMatrix),
    ctx,
    REGION_FILTER_MAPPINGS.phase10Urgency,
    warnings,
    'UrgencyAgeMatrix'
  ...
  const employmentMatrix = filterByRegion(
    readFirstAvailableSheet(REGION_DASHBOARD_SHEETS.phase10.employmentMatrix),
    ctx,
    REGION_FILTER_MAPPINGS.phase10Urgency,
    warnings,
    'UrgencyEmploymentMatrix'
  );
  const municipalityData = filterByRegion(
    readFirstAvailableSheet(REGION_DASHBOARD_SHEETS.phase10.byMunicipality),
    ctx,
    REGION_FILTER_MAPPINGS.phase10Urgency,
    warnings,
    'UrgencyByMunicipality'
  );
  ...
  return {
    ...
    tables: {
      ...
      quality: filterByRegionIfPossible(quality, ctx, REGION_FILTER_MAPPINGS.phase10Urgency)
    },
    warnings: warnings
  };
}
