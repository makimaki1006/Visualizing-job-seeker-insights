...
function getAvailablePrefectures() {
  Logger.log('[getAvailablePrefectures] 開始');
  ...
  Logger.log('[getAvailablePrefectures] 読み取り行数: ' + rows.length);

  if (!rows.length) {
    Logger.log('[getAvailablePrefectures] シートが空です');
    return [];
  }

  const prefectureIndex = findColumnIndex(rows[0], REGION_COLUMN_LABELS.PREFECTURE);
  Logger.log('[getAvailablePrefectures] prefectureIndex: ' + prefectureIndex);

  if (prefectureIndex === -1) {
    Logger.log('[getAvailablePrefectures] 都道府県カラムが見つかりません');
    return [];
  }
  ...
  Logger.log('[getAvailablePrefectures] 都道府県リスト（最初の10件）: ' + JSON.stringify(prefectures.slice(0, 10)));
...
function getMunicipalitiesForPrefecture(prefecture) {
  Logger.log('[getMunicipalitiesForPrefecture] 開始 - prefecture: "' + prefecture + '"');
  ...
  // 🚀 最適化: prefecture で絞り込んでシート読み込み（高速化）
  const rows = readSheetRows(REGION_SOURCE_SHEETS.MAP_METRICS, prefValue);
  Logger.log('[getMunicipalitiesForPrefecture] 読み取り行数（prefecture絞り込み後）: ' + rows.length);

  if (!rows.length) {
    Logger.log('[getMunicipalitiesForPrefecture] シートが空です');
    return [];
  }
  ...
  Logger.log('[getMunicipalitiesForPrefecture] prefIndex: ' + prefIndex + ', muniIndex: ' + muniIndex);
  ...
  // デバッグ: 最初の5行の都道府県値を表示
  Logger.log('[getMunicipalitiesForPrefecture] サンプル行（最初の5件）:');
  rows.slice(1, 6).forEach((row, i) => {
    Logger.log('  行' + (i + 1) + ': prefecture="' + row[prefIndex] + '", municipality="' + row[muniIndex] + '"');
  });
  ...
  const municipalities = Array.from(
    new Set(
      rows.slice(1)
        .filter(row => {
          const rowPref = normalizeRegionValue(row[prefIndex]);
          return rowPref === prefValue;
        })
  ...
  Logger.log('[getMunicipalitiesForPrefecture] 抽出された市区町村数: ' + municipalities.length);
  Logger.log('[getMunicipalitiesForPrefecture] 市区町村リスト（最初の10件）: ' + JSON.stringify(municipalities.slice(0, 10)));
